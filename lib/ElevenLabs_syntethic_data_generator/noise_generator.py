...
A generált zajok négy környezethez készülnek:
1. Iroda (--office)
2. Bár (--bar)
3. Utca (--street)
4. Otthon (--home)
...
python noise_generator.py --bar 5 --office 3
...
def parse_arguments():
    ...
    if not any([args.office, args.bar, args.street, args.home]):
        parser.print_help()
        print("\nFigyelmeztetés: Nincs megadva generálandó környezet.")
        print("Használja a --office, --bar, --street vagy --home flageket a zajok generálásához.")
        print("Példa: python noise_generator.py --bar 5 --office 3")
        sys.exit(1)
